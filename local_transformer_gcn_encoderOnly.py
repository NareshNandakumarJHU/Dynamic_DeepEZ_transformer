...
class BrainNetCNN(torch.nn.Module):
    def __init__(self, example, num_classes=10):
        ...
        self.encoder_layer = torch.nn.TransformerEncoderLayer(d_model=2460,nhead=4)
        self.trans_encoder = torch.nn.TransformerEncoder(self.encoder_layer,num_layers=2)
    ...
    def forward(self, x, adj):
        ...
        encoded = self.trans_encoder(trans_reshape)

        #Attn = self.trans(trans_reshape,trans_reshape)

        Attn = F.leaky_relu(self.temporalFC1(encoded),negative_slope=0.1)
